...
void PatoInterfaz::pato_camina_derecha(SDL2pp::Renderer& renderer, SDL_Rect& rect_inicio, SDL2pp::Rect& rect_destino) {
...
void PatoInterfaz::pato_camina_izquierda(SDL2pp::Renderer& renderer, SDL_Rect& rect_inicio, SDL2pp::Rect& rect_destino) {
    for (int i = 0; i < NUM_FRAMES_MOVIMIENTO_PATO; ++i) {
        renderer.Clear();
        rect_destino.x -= 2;
...
void PatoInterfaz::pato_salta(SDL2pp::Renderer& renderer, SDL_Rect& rect_inicio, SDL2pp::Rect& rect_destino) {
    for (int i = 0; i < NUM_FRAMES_SALTA_PATO; ++i) {
        rect_destino.y -= 3;
        rect_destino.x += 0.5;
        renderer.Copy(salto_pato[i], SDL2pp::Optional<SDL2pp::Rect>(rect_inicio), SDL2pp::Optional<SDL2pp::Rect>(rect_destino));
        renderer.Present();
        SDL_Delay(70);
    }
    ...
    renderer.Copy(salto_pato[0], SDL2pp::Optional<SDL2pp::Rect>(rect_inicio), SDL2pp::Optional<SDL2pp::Rect>(rect_destino));
    renderer.Present();
}

void PatoInterfaz::pato_agachado(SDL2pp::Renderer& renderer, SDL_Rect& rect_inicio, SDL2pp::Rect& rect_destino) {
    for (int i = 0; i < NUM_FRAMES_PATO_ACOSTADO; ++i) {
        renderer.Copy(acostarse_pato[i], SDL2pp::Optional<SDL2pp::Rect>(rect_inicio), SDL2pp::Optional<SDL2pp::Rect>(rect_destino));
        renderer.Present();
        SDL_Delay(150);
    }
    renderer.Clear();
    renderer.Copy(acostarse_pato[0], SDL2pp::Optional<SDL2pp::Rect>(rect_inicio), SDL2pp::Optional<SDL2pp::Rect>(rect_destino));
    renderer.Present();
}

SDL2pp::Texture& PatoInterfaz::mostrar_frame() {
    return movimiento_pato[0];
}

void PatoInterfaz::dibujar(SDL2pp::Renderer& renderer, SDL_Rect& rect_inicio, SDL2pp::Rect& rect_destino) {
...
void PatoInterfaz::cargar_frames(SDL2pp::Renderer& renderer, SDL2pp::Surface& sprite_sheet, int offset_y, std::vector<SDL2pp::Texture>& texturas, int num_frames) {
    for (int i = 0; i < num_frames; ++i) {
        
        SDL_Rect rect_inicial = {i * PIXEL_PATO, offset_y, PIXEL_PATO, PIXEL_PATO};
        
        SDL2pp::Surface sprite_superficie(SDL_CreateRGBSurface(0, PIXEL_PATO, PIXEL_PATO, 32, 0, 0, 0, 0));
